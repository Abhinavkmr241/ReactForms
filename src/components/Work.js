...
class Work extends Component {
    ...
    componentDidMount() {
        getUserData().then(resp => {
            ...
            let docTime = resp.doctor.availability;
            for (let i of docTime) {
                for (let j of this.state.serviceTime) {
                    if (i.from === j.value) {
                        i.from = j.label
            ...
            for (let d of docTime) {
                days[d.day.toLowerCase()].push({ fromTime: d.from, toTime: d.to })
            }
            this.setState({
                days: {
                    monday: days.monday,
